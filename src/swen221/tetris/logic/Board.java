...
public class Board {
  ...
  //reverseRangeY gives us a stream of all the y coordinates in reverse order
  //we then use .map which takes each element of Y and returns printLine(y) for each element
  //using collect and collectors.joining allows us to return a single string representing the board
  // the \n on each end of collectors.joining will result in printing each row of the board on a seperate line
  ...
  //rangeX gives us a stream of all the x coordinates
  //we use .map which gives us each element of x 
  //we use that to get the string of that specific cell ( it can return a colour or be empty )
  //we then use .collect and collectors.joining to return a single string for that row
